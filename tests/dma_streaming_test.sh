...
do
  ...
  $tool_path/dma_from_device -d /dev/sqrldma0_c2h_${i} -f data/output_datafile${i}_4K.bin -s $transferSize -c $transferCount &
done
...
do
  ...
  $tool_path/dma_to_device -d /dev/sqrldma0_h2c_${i} -f data/datafile${i}_4K.bin -s $transferSize -c $transferCount &
done
